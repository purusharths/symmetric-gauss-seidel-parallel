...
void benchmark(int k, int blocksize) {
  ...
  for (auto n : sizes) {
    ...
    std::cout << "blocked, " << n << ", " << d.second / time_factor << ", "
              << d.first << ", " << flops << ", " << flops * sizeof(NUMBER)
              << std::endl;
  }
  // std::cout << "\n\n----Experiment Over---- \n\n";
  ...
  for (auto n : sizes) {
    ...
    std::cout << "simd omp, " << n << ", " << d.second / time_factor << ", "
              << d.first << ", " << flops << ", " << flops * sizeof(NUMBER)
              << std::endl;
  }
  // std::cout << "\n\n----Experiment Over---- \n\n";
  ...
  for (auto n : sizes) {
    ...
    auto d = time_experiment(e);
    double flops = d.first * e.operations() / d.second * 1e6 / 1e9;
    std::cout << "simd 4d, " << n << ", " << d.second / time_factor << ", "
              << d.first << ", " << flops << ", " << flops * sizeof(NUMBER)
              << std::endl;
  }
  // std::cout << "\n\n----Experiment Over---- \n\n";
  ...
  for (auto n : sizes) {
    ...
    if (k > 6) {
      ...
      auto d = time_experiment(e);
      double flops = d.first * e.operations() / d.second * 1e6 / 1e9;
      std::cout << "simd 8d, " << n << ", " << d.second / time_factor << ", "
                << d.first << ", " << flops << ", " << flops * sizeof(NUMBER)
                << std::endl;
    } else {
      std::cout << "# Stencil smaller than vector size. Skipping..."
                << std::endl;
    }
...
int main(int argc, char **argv) {
  ...
  if (argc < 2 || argc > 3) {
    std::cout
        << "usage: ./bm_stencil <stencil size (k)> <block size>\n" << std::endl;
    exit(1);
  }
  sscanf(argv[1], "%d", &k);
